...
**Codex CLI：** このリポジトリをCodexプラグインとしてインストール（標準Claude Code / Codex互換スキルプラグイン、特別な設定不要）。
...
隔離ワークスペース内でも同様に適用されます。`"This looks like a secret file"`が表示されたら、バグではなくガードが正常に動作している証拠です。ガードフックはClaude Codeでのみ実行されます。Codexではサンドボックスがこの役割を担います。
