...
スキルのみ使用されます（Claude Codeフックはcodexでは実行されないため無視して構いません）。
Codexローダーはネストされたパスのスキルを検出しますが、標準インストールパス（`~/.codex/skills/<name>/SKILL.md`）とは異なるバンドル構造です。
...
- **Codex CLI** — `/senpai`で明示的に呼び出してください。他のスキルが一緒にインストールされている場合、自動ルーティングが別のスキルを優先する可能性があります。直接呼び出しが確実です。

> **Codexで毎回`/senpai`と入力したくない場合：** プロジェクトフォルダに`AGENTS.md`ファイルを作り、以下の一行を入れてください。コーディング要求が自動的にsenpaiを経由します。
>
> ```
