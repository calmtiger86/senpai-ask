...
코딩도, 터미널도, 바이브코딩도 처음인 사람을 위한 Claude Code / Codex CLI **Brainstorming 방식의 작업 설계 스킬.**
...
**2. 전문용어가 어렵다.**
> 격리(원본을 건드리지 않는 별도 복사본), 커밋(작업 저장점)처럼, 낯선 단어가 나올 때 **괄호로 한 번만 설명**하고 이후엔 반복하지 않습니다.

**3. 작업 기록을 남기는 방법을 모른다.**
> "지난번에 어디까지 했지?"라고 물으면, 기술 로그가 아니라 사람 말로 **"로그인 페이지 디자인까지 정했고, 다음은 회원가입 흐름"** 이라고 알려줍니다.

이 세 가지 외에는 Claude Code / Codex의 네이티브 기능(권한 확인, 되돌리기, 샌드박스)을 대체하지 않고 그대로 사용합니다.
