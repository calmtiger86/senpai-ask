...
**Codex CLI:** 이 저장소를 Codex 플러그인으로 설치 (표준 Claude Code / Codex 호환 스킬 플러그인이라 별도 설정 불필요).
...
격리 공간 안에서도 동일하게 적용됩니다. `"This looks like a secret file"` 메시지가 뜨면 버그가 아니라 이 가드가 정상 작동하는 것입니다. 가드 훅은 Claude Code에서만 실행됩니다. Codex에서는 sandbox가 이 역할을 대신합니다.
