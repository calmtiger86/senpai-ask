...
为从未写过代码、从未用过终端、从未进行过 vibe-coding 的人准备的 Claude Code / Codex CLI **头脑风暴式的工作设计技能。**
...
**2. 专业术语太难。**
> 隔离（不碰原始文件的独立副本）、提交（工作存档点）——陌生词汇第一次出现时**用括号解释一次**，之后不再重复。

**3. 不知道怎么留工作记录。**
> 问"上次做到哪了？"，它不会给你技术日志，而是用人话说：**"登录页设计已确定，下一步是注册流程。"**

除了这三点，不替代 Claude Code / Codex 的原生功能（权限确认、回退、沙盒）——直接使用它们。
